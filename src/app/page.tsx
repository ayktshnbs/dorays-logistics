...
const pageCopy = {
  en: {
    ...
    services: [
      ['local_shipping', 'Road Freight', 'Full truckload (FTL) and less-than-truckload (LTL) services across Europe and the Middle East with real-time GPS tracking.'],
      ['directions_boat', 'Sea Freight', 'Cost-effective global ocean transport for containerized cargo, bulk shipments, and specialized reefers.'],
      ['flight', 'Air Freight', 'Express air transport for time-sensitive cargo, high-value electronics, and emergency medical supplies.'],
      ['gavel', 'Customs Coordination', 'Expert brokerage and compliance management to ensure smooth border crossings in every jurisdiction.'],
      ['inventory_2', 'Warehousing', 'Strategically located hubs offering inventory management, cross-docking, and last-mile distribution.'],
      ['precision_manufacturing', 'Project Cargo', 'Specialized heavy-lift and out-of-gauge transport for industrial machinery and energy infrastructure.'],
    ],
    learnMore: 'LEARN MORE',
    processBadge: 'OUR PROCESS',
    processTitle: 'A transparent workflow',
    processSteps: [
      ['01', 'Route & cargo analysis', "We evaluate your shipment's requirements, urgency, and destination to determine the optimal route."],
      ['02', 'Offer & transport planning', 'Direct quotes and meticulous planning including customs documentation and modal integration.'],
      ['03', 'Operational follow-up', '24/7 tracking and coordination by our expert team until the cargo reaches its final destination.'],
    ],
    ...
    footerText: 'Empowering global trade through technological precision and operational excellence.',
    footerColumns: [
      ['SERVICES', ['Road Transport', 'Sea Logistics', 'Air Cargo', 'Supply Chain Solutions']],
      ['COMPANY', ['About Us', 'Network Map', 'Careers', 'Contact']],
      ['RESOURCES', ['Track Shipment', 'Documentation Guide', 'Incoterms 2024', 'Fuel Surcharges']],
    ],
  ...
  tr: {
    ...
    services: [
      ['local_shipping', 'Kara Yolu Taşımacılığı', 'Avrupa ve Orta Doğu genelinde gerçek zamanlı GPS takibiyle komple (FTL) ve parsiyel (LTL) taşıma hizmetleri.'],
      ['directions_boat', 'Deniz Yolu Taşımacılığı', 'Konteyner yükleri, dökme sevkiyatlar ve özel reefer çözümleri için maliyet etkin global deniz taşımacılığı.'],
      ['flight', 'Hava Yolu Taşımacılığı', 'Zaman hassasiyetli yükler, yüksek değerli elektronik ürünler ve acil medikal sevkiyatlar için ekspres hava kargo.'],
      ['gavel', 'Gümrük Koordinasyonu', 'Her bölgede sorunsuz sınır geçişi için uzman brokerlik ve mevzuat uyum yönetimi.'],
      ['inventory_2', 'Depolama', 'Envanter yönetimi, cross-docking ve son kilometre dağıtımı sunan stratejik konumlu lojistik merkezleri.'],
      ['precision_manufacturing', 'Proje Taşımacılığı', 'Endüstriyel makineler ve enerji altyapısı için ağır, gabari dışı ve özel yük taşıma planlaması.'],
    ],
    learnMore: 'DETAYLAR',
    processBadge: 'SÜRECİMİZ',
    processTitle: 'Şeffaf bir iş akışı',
    processSteps: [
      ['01', 'Rota ve yük analizi', 'Sevkiyatın gereksinimlerini, aciliyetini ve varış noktasını değerlendirerek en uygun rotayı belirleriz.'],
      ['02', 'Teklif ve taşıma planı', 'Gümrük dokümantasyonu ve mod entegrasyonu dahil net teklifler ve titiz operasyon planlaması sunarız.'],
      ['03', 'Operasyon takibi', 'Yük nihai varış noktasına ulaşana kadar uzman ekibimizle 7/24 takip ve koordinasyon sağlarız.'],
    ],
    ...
    footerText: 'Küresel ticareti teknolojik hassasiyet ve operasyonel mükemmellikle güçlendiriyoruz.',
    footerColumns: [
      ['HİZMETLER', ['Kara Taşımacılığı', 'Deniz Lojistiği', 'Hava Kargo', 'Tedarik Zinciri Çözümleri']],
      ['ŞİRKET', ['Hakkımızda', 'Ağ Haritası', 'Kariyer', 'İletişim']],
      ['KAYNAKLAR', ['Sevkiyat Takibi', 'Dokümantasyon Rehberi', 'Incoterms 2024', 'Yakıt Ek Ücretleri']],
    ],
...
export default function Home() {
  ...
  return (
    <main className={`min-h-screen text-[#eae1d8] transition-colors ${isLight ? 'light-site bg-[#f7f5f2]' : 'dark-site bg-[#110e09]'}`}>
      <header className={`sticky top-0 z-50 flex min-h-12 w-full items-center justify-between gap-4 border-b px-4 py-2 text-[13px] shadow-sm sm:px-8 ${isLight ? 'border-black/10 bg-[#f7f5f2]' : 'border-[#4f4538] bg-[#110e09]'}`}>
        ...
        <nav className="hidden items-center gap-8 md:flex">
          {copy.nav.map(([item, href], index) => (
            <a key={item} className={index === 0 ? 'font-bold text-[#c88919]' : 'text-slate-400 transition hover:text-[#c88919]'} href={href}>
              {item}
            </a>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={themeLabel}
          >
            <Icon className="icon-control-symbol">{isLight ? 'dark_mode' : 'light_mode'}</Icon>
            <span className="sr-only">{themeLabel}</span>
          </button>
          <a href="#contact" className="inline-flex h-9 items-center rounded bg-[#ffce80] px-4 text-xs font-extrabold text-[#432c00] transition hover:shadow-[0_0_18px_rgba(255,206,128,0.28)]">
            {copy.quote}
          </a>
        </div>
      </header>

      <section id="home" className="relative flex min-h-[91vh] items-center overflow-hidden px-4 py-16 sm:px-8 lg:px-12">
        <div className="absolute inset-0 bg-black/45">
          <img src={heroImage} alt="Container port at dusk" className="h-full w-full object-cover" />
        ...
        <div className="relative z-10 mx-auto grid w-full max-w-7xl gap-12 lg:grid-cols-12">
          <div className="flex flex-col justify-center lg:col-span-6">
            <span className="mb-4 text-xs font-extrabold uppercase tracking-[0.08em] text-[#ffce80]">{copy.heroBadge}</span>
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <a href="#contact" className="inline-flex min-h-12 items-center gap-2 rounded bg-[#ffce80] px-8 text-sm font-bold text-[#432c00] transition hover:shadow-[0_0_24px_rgba(255,206,128,0.32)]">
                {copy.heroPrimary}
                <Icon className="text-[20px]">arrow_forward</Icon>
              </a>
              <a href="#services" className="inline-flex min-h-12 items-center rounded border border-[#9b8f7f] bg-[#16130e]/25 px-8 text-sm font-bold text-white backdrop-blur-sm transition hover:bg-[#231f19]">
                {copy.heroSecondary}
              </a>
            </div>
          </div>
          ...
          <div className="relative min-h-[230px] lg:col-span-6">
            <div className="absolute right-0 top-4 hidden h-[330px] w-[420px] opacity-20 lg:block">
              <img src={routeMapImage} alt="Abstract Eurasian route map" className="h-full w-full object-contain" />
            </div>
            <div className="relative z-20 ml-auto max-w-md rounded border border-[#4f4538] bg-[#231f19] p-6 shadow-[0_4px_20px_-5px_rgba(255,206,128,0.18)] lg:mt-20">
              <div className="mb-6 flex items-center justify-between">
                <span className="text-xs font-extrabold uppercase tracking-[0.08em] text-[#ffce80]">{copy.shipmentLabel}</span>
                <span className="rounded bg-[#ffce80]/20 px-2 py-1 text-xs font-bold text-[#ffce80]">{copy.transit}</span>
              </div>
              <div className="mb-8 flex items-center justify-between gap-5">
                ...
                <div className="relative h-px flex-1 bg-[#4f4538]">
                  ...
                  <Icon className="absolute -top-5 left-1/3 -translate-x-1/2 text-[20px] text-[#ffce80]">local_shipping</Icon>
                </div>
                <div className="text-center">
      ...
      <section className="border-y border-[#4f4538] bg-[#1f1b16] px-4 py-8 sm:px-8">
        <div className="mx-auto grid max-w-7xl gap-6 text-center md:grid-cols-3">
          {copy.stats.map(([value, label]) => (
            <div key={label}>
              <span className="block text-2xl font-bold text-[#ffce80]">{value}</span>
              <span className="mt-1 block text-xs font-bold uppercase tracking-[0.08em] text-[#d2c4b3]">{label}</span>
            </div>
          ))}
        </div>
      </section>

      <section id="services" className="bg-[#f7f5f2] px-4 py-12 text-[#231f19] sm:px-8 lg:px-12">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10">
            <span className="text-xs font-extrabold uppercase tracking-[0.08em] text-[#eebf72]">{copy.servicesBadge}</span>
            <h2 className="mt-2 text-4xl font-extrabold leading-tight text-[#e8ded5] sm:text-5xl">{copy.servicesTitle}</h2>
          </div>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {copy.services.map(([icon, title, text]) => (
              <article key={title} className="group rounded border border-[#110e09] bg-[#2e2923] p-6 text-white transition hover:border-[#ffce80]">
                <Icon className="mb-5 text-[36px] text-[#ffce80]">{icon}</Icon>
                <h3 className="mb-3 text-lg font-bold">{title}</h3>
                <p className="mb-6 min-h-[72px] text-sm leading-6 text-[#eae1d8]">{text}</p>
                <a href="#" className="inline-flex items-center gap-1 text-xs font-extrabold text-[#ffce80] transition group-hover:gap-2">
                  {copy.learnMore} <Icon className="text-[16px]">chevron_right</Icon>
                </a>
              </article>
            ))}
          </div>
      ...
      <section id="process" className="bg-[#1a1712] px-4 py-12 sm:px-8 lg:px-12">
        <div className="mx-auto max-w-7xl">
          <div className="mx-auto mb-10 max-w-2xl text-center">
            <span className="text-xs font-extrabold uppercase tracking-[0.08em] text-[#ffce80]">{copy.processBadge}</span>
            <h2 className="mt-2 text-4xl font-extrabold text-white sm:text-5xl">{copy.processTitle}</h2>
          </div>
          <div className="relative grid gap-10 md:grid-cols-3">
            ...
            {copy.processSteps.map(([number, title, text]) => (
              <article key={number} className="relative text-center">
                ...
                <p className="mx-auto max-w-sm text-sm leading-6 text-[#d2c4b3]">{text}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section id="network" className="overflow-hidden bg-[#f7f5f2] px-4 py-12 text-[#231f19] sm:px-8 lg:px-12">
        <div className="mx-auto flex max-w-7xl flex-col items-center gap-12 lg:flex-row">
          <div className="lg:w-1/2">
            <span className="text-xs font-extrabold uppercase tracking-[0.08em] text-[#eebf72]">{copy.networkBadge}</span>
            <h2 className="mt-2 max-w-lg text-4xl font-extrabold leading-tight text-[#e8ded5] sm:text-5xl">{copy.networkTitle}</h2>
            <p className="mt-6 max-w-xl text-base leading-7 text-[#d2c4b3]">
              {copy.networkText}
            </p>
            <div className="mt-9 grid gap-5 sm:grid-cols-2">
              {copy.networkLinks.map(([icon, label]) => (
                <div key={label} className="flex items-center gap-3 text-sm font-semibold text-[#d2c4b3]">
          ...
          <div className="lg:w-1/2">
            <div className="rounded border border-[#4f4538] bg-[#231f19] p-2">
              <img src={networkImage} alt="Digital global cargo route map" className="w-full rounded object-cover" />
            </div>
          </div>
        </div>
      </section>

      <section id="contact" className="bg-[#1f1b16] px-4 py-12 sm:px-8 lg:px-12">
        <div className="mx-auto max-w-7xl">
          <div className="overflow-hidden rounded border border-[#4f4538] bg-[#231f19] lg:flex">
            <div className="flex flex-col justify-between bg-[#ffce80] p-10 text-[#432c00] lg:w-2/5">
              <div>
                <h2 className="mb-4 text-3xl font-extrabold">{copy.contactTitle}</h2>
            ...
            >
              {copy.formFields.map(([label, placeholder, type]) => (
                <label key={label} className="flex flex-col gap-1">
                  <span className="text-xs font-extrabold uppercase tracking-[0.08em] text-[#9b8f7f]">{label}</span>
                  {type === 'select' ? (
                    <select required className="rounded border border-[#4f4538] bg-[#16130e] px-3 py-2.5 text-sm text-[#eae1d8] outline-none focus:border-[#ffce80] focus:ring-1 focus:ring-[#ffce80]">
                      {copy.freightOptions.map((option) => (
                        <option key={option}>{option}</option>
                      ))}
                    </select>
                  ) : (
                    <input required type={type} placeholder={placeholder} className="rounded border border-[#4f4538] bg-[#16130e] px-3 py-2.5 text-sm text-[#eae1d8] outline-none placeholder:text-[#9b8f7f] focus:border-[#ffce80] focus:ring-1 focus:ring-[#ffce80]" />
                  )}
                </label>
              ))}
              <label className="flex flex-col gap-1 md:col-span-2">
                <span className="text-xs font-extrabold uppercase tracking-[0.08em] text-[#9b8f7f]">{copy.message}</span>
                <textarea required rows={4} placeholder={copy.messagePlaceholder} className="rounded border border-[#4f4538] bg-[#16130e] px-3 py-2.5 text-sm text-[#eae1d8] outline-none placeholder:text-[#9b8f7f] focus:border-[#ffce80] focus:ring-1 focus:ring-[#ffce80]" />
              </label>
              <button type="submit" className="min-h-12 rounded bg-[#ffce80] text-sm font-bold text-[#432c00] transition hover:shadow-[0_0_20px_rgba(255,206,128,0.22)] md:col-span-2">
                {copy.send}
              </button>
              {submitted && (
      ...
      <footer className="border-t border-[#4f4538] bg-[#110e09] px-4 py-12 sm:px-8 lg:px-12">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 grid gap-10 md:grid-cols-4">
            <div>
              <img src="/dorays-logo.jpg" alt="Dorays Logistics Logo" className="mb-4 h-10 w-auto object-contain" />
              <p className="max-w-xs text-sm leading-6 text-[#9b8f7f]">{copy.footerText}</p>
            </div>
            {copy.footerColumns.map(([title, links]) => (
              <div key={title as string}>
                <h4 className="mb-4 text-xs font-extrabold uppercase tracking-[0.08em] text-[#eae1d8]">{title}</h4>
          ...
          <div className="flex flex-col items-center justify-between gap-4 border-t border-[#4f4538] pt-6 text-xs text-[#9b8f7f] md:flex-row">
            ...
            <div className="flex gap-6">
              {copy.legal.map((item) => (
                <a key={item} href="#" className="hover:text-[#ffce80]">{item}</a>
              ))}
            </div>
          </div>
        </div>
